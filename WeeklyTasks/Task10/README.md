...
### Tactical Bot
- Functions on an attacking mode and defensive mode
- Detects incoming enemy fleets and sends help from nearby planets.
- Chooses an available source planet with the maximum number of ships to attack
- Targets an enemy or neutral planet with the minimum number of ships
- Falls back to neutral planets when no enemies can be detected from fog of war
- Sends 75% of the source's ships when it holds more than 10
- Uses lambda functions with Python's min/max for clean target selection
